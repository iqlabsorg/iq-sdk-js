...
import { Enterprise, EnterpriseFactory, ERC20Mock } from '../../types/contracts';
...
describe('@iqprotocol/ethereum', () => {
  describe('EthereumBlockchainProvider', () => {
    ...
    it('works', async () => {
      ...
      const balance = await ethereumProvider.getBalance(erc20Token.address, deployerAddress);
    ...
    describe('When enterprise deployed', () => {
      ...
      beforeEach(async () => {
        ...
        const receipt = await tx.wait(1);
        enterprise = (await getEnterprise(enterpriseFactory, receipt.blockNumber)) as Enterprise;
      });

      it('retrieves enterprise data', async () => {
        const data = await ethereumProvider.getEnterpriseData(enterprise.address);
