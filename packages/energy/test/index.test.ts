...
describe('Energy Utils', () => {
  describe('halfLife', () => {
    ...
    `(
      'correct when: initialValue = $initialValue, halfLifePeriod = $gapHalvingPeriod, t0 = $t0, t1 = $t1',
      ({ initialValue, gapHalvingPeriod, t0, t1, expected }) => {
  ...
  describe('energy cap', () => {
    test.each`
      power   | prevEnergyCap | gapHalvingPeriod           | t0   | t1     | expected
      ${100n} | ${0n}         | ${50}                      | ${0} | ${0}   | ${0n}
      ${100n} | ${0n}         | ${50}                      | ${0} | ${25}  | ${29n}
      ${100n} | ${0n}         | ${50}                      | ${0} | ${50}  | ${50n}
    ...
    `(
      'correct when: power = $power, prevEnergyCap = $prevEnergyCap, gapHalvingPeriod = $gapHalvingPeriod, t0 = $t0, t1 = $t1',
      ({ power, prevEnergyCap, gapHalvingPeriod, t0, t1, expected }) => {
        // eslint-disable-next-line @typescript-eslint/no-unsafe-assignment
        expect(calculateEnergyCap({ power, prevEnergyCap, gapHalvingPeriod: gapHalvingPeriod, t0, t1 })).toEqual(
          expected,
        );
      },
    );
